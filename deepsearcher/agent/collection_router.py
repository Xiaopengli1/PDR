...
COLLECTION_ROUTE_PROMPT = """I provide you with collection_name(s) and corresponding collection_description(s). Please select the collection names that may be related to the question and return a python list of str. If there is no collection related to the question, you can return an empty list.

"QUESTION": {question}
"COLLECTION_INFO": {collection_info}

When you return, you need to put your reasoning process in <think> </think> and return the collection names in <output> </output> with a python list of str.
"""
...
class CollectionRouter(BaseAgent):
    ...
    def invoke(self, query: str, dim: int, **kwargs) -> Tuple[List[str], int]:
        ...
        selected_collections, _ = self.llm.literal_eval(output_content)
        ...
        log.color_print(
            f"<local search> Perform search [{query}] on the local vector DB collections: {selected_collections} </local search>\n"
        )
