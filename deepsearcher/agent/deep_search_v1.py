...
class DeepSearch(RAGAgent):
    ...
    def __init__(
        ...
        personalized_info_address: str = '/home/xiaopli2/DeepSearch-main/examples/personalized_summary.json',
        max_iter: int = 3,
    ...
    def _search_chunks_from_mix(self, query: str, sub_queries: List[str]):
        ...
        for collection in selected_collections:
            log.color_print(f"<local search> Search [{query}] in [{collection}]...  </local search>\n")
            retrieved_results = self.vector_db.search_data(
                collection=collection, vector=query_vector, top_k=2
            )
            if not retrieved_results or len(retrieved_results) == 0:
                log.color_print(
                    f"<local search> No relevant document chunks found in '{collection}' related to query! </local search>\n"
                )
                continue
            else:
                log.color_print(
                    f"<local search> Retrieve {len(retrieved_results)} relevant chunks in '{collection}'! </local search>\n"
                )
            ...
            for retrieved_result in retrieved_results:
                ...
                if "<think>" in response_content and "</think>" in response_content:
                    ...
                    log.color_print(f"<think> {thinking_process} </think>\n")
            ...
            if accepted_chunk_num > 0:
                log.color_print(
                    f"<local search> Accept {accepted_chunk_num} document chunk(s) from references: {list(references)} </local search>\n"
                )
            else:
                log.color_print(
                    f"<local search> No document chunk accepted from '{collection}'! </local search>\n"
                )
            all_accepted_chunk_num += accepted_chunk_num

        # local search overall evaluation
        ...
        if "YES" in output_content and "NO" not in output_content:
            ...
            log.color_print(
                f"<web seach> let's search query {revised_query_content} on website <web seach>\n"
            )
        ...
        if all_accepted_chunk_num < 1:
            log.color_print(
                f"<web search> Conduct web search </web search>\n"
            )
        results = api.get_sources(query, num_results = 2)
        num_web_results = len(results.data["organic"])
        web_search_results = []

        for index in range(num_web_results):
            web_search_results.append(results.data['organic'][index]['snippet'])


        accepted_web_chunk_num = 0

        web_search_results_new = []
        for retrieved_web_result in web_search_results:

            log.color_print(
                f"<web search result> Get search result: [{retrieved_web_result}] </web search result>\n"
            )
            chat_response = self.llm.chat(
                messages=[
                    {"role": "system", "content": SYSTEM_PROMPT.format(personalized_info=self.personalized_info)},
                    {"role": "user", "content": RERANK_WEB_PROMPT.format(
                        query=[query] + sub_queries,
                        retrieved_chunk=f"<chunk>{retrieved_web_result}</chunk>", ),
                     }
                ]
            )
            consume_tokens += chat_response.total_tokens
            ...
            if "YES" in response_content and "NO" not in response_content:
                log.color_print(f"accepted web chunk [{retrieved_web_result}] </web search>\n")
                web_search_results_new.append(retrieved_web_result)
                accepted_web_chunk_num += 1
            else:
                log.color_print(f"reject web chunk [{retrieved_web_result}] </web search>\n")

        all_retrieved_results.extend(web_search_results_new)
    ...
    def async_retrieve(
        ...
        for iter in range(max_iter):
            ...
            ################# Prarallel operation #################
            # search_tasks_online = [
            #     self._search_chunks_from_mix(query, sub_gap_queries)
            #     for query in sub_gap_queries
            # ]
            #
            # # Execute all tasks in parallel and wait for results
            # search_results = await asyncio.gather(*search_tasks_online)
            ...
            # search_res_from_vectordb, search_res_from_internet
    ...
    def query(self, query: str, **kwargs) -> Tuple[str, List[RetrievalResult], int]:
        ...
        # Preference = {
        # "User Content Preferences": "Prefers in-depth analysis of social issues, values humanistic care, and favors constructive discussions.",
        # "User Behavior Patterns": "Tends to provide detailed background information and professional analysis, with responses that are usually rigorous and logically structured."}
